    ...
    <div className="space-y-6">
      ...
      <div className="relative w-full">
        ...
        <Input
          ...
          className="pl-12 h-14 bg-white border-none shadow-md text-lg rounded-2xl focus-visible:ring-amber-500"
          value={search}
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-4 py-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="note-phone">Phone Number</Label>
                <Input
                  id="note-phone"
            ...
            <div className="space-y-2">
              ...
              <textarea
                id="note-content"
                className="w-full min-h-[150px] p-3 rounded-md border bg-white focus:outline-none focus:ring-1 focus:ring-amber-400"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredNotes.length === 0 ? (
          <div className="col-span-full text-center py-12 text-slate-500 bg-white/30 rounded-2xl border border-dashed border-slate-300">
            No notes found
          </div>
        ) : (
          filteredNotes.map((note) => (
            <Card key={note.id} className="border-none shadow-md bg-white hover:shadow-lg transition-shadow rounded-2xl overflow-hidden">
              <CardHeader className="pb-2 flex flex-row items-start justify-between space-y-0">
                <div className="space-y-1">
                  <CardTitle className="text-lg font-bold text-slate-900">{note.customerName}</CardTitle>
                  <p className="text-xs text-slate-500">{note.phoneNumber}</p>
                </div>
                <Button
                  variant="ghost"
                  size="icon"
              ...
              <CardContent>
                <p className="text-sm text-slate-700 whitespace-pre-wrap mb-4 line-clamp-6">{note.content}</p>
